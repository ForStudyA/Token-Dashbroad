...
2026-06-30 | 去掉 MiMo 硬编码路由，改为通用 provider + 映射机制；模型映射加 protected 字段，原样转发不可删除
2026-06-30 | UI: 原样转发移至提供商旁，与 provider 互斥；启用按钮始终显示；去掉顶部模型筛选；刷新移至代理开关右侧
2026-06-30 | 日志显示 request->actual 模型映射，延迟改为秒，最近请求移至模型映射下方，流式响应捕获实际模型名
2026-06-30 | 代理面板固定高度布局，日志来源动态标签，映射显示 source->target
2026-06-30 | 回退：移除 display_model/sourceLabel，恢复内联写法和固定高度布局
2026-07-01 | 禁用代理兼容转发（不再 503）、启动预加载同步代理状态、provider 已有 key 时不覆盖、测试按钮防连点、映射优先于 MiMo 兜底
